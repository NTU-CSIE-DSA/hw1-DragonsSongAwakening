...
void incident() {
  ...
  if (type == 1) {
    ...
    scanf("%d", &b);
    scanf("%d", &m);
    if (m == M || (int)player[b].n_record <= m) printf("%lld\n", player[b].spending);
    else {
      ll tmp = player[b].spending;
      Node *tmpc = player[b].cursor->nxt;
      int n_rec = ((int)player[b].n_record < M)? (int)player[b].n_record : M;
      for (int i = 0; i < n_rec-m; i++) {
        tmp -= tmpc->value;
        tmpc = tmpc->nxt;
      }
      printf("%lld\n", tmp);
