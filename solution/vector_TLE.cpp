...
void incident() {
  ...
  if (type == 1) {
    ...
    int b; cin >> b;
    int m; cin >> m;
    if (m == M || (int)player[b].record.size() <= m) cout << player[b].spending << '\n';
    else {
      ll tmp = player[b].spending;
      for (int i = 0, j = max(0, (int)player[b].record.size()-M); i < min((int)player[b].record.size(), M)-m; i++, j++) tmp -= player[b].record[j];
      cout << tmp << '\n';
    }
  } else cout << "ERROR\n";
}
