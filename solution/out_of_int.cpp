...
struct Classmate {
  ...
  int update() {
    p += (int)(N - rank) * (n_up - last_up);
    assert((int)(N - rank) * (n_up - last_up) >= 0);
    last_up = n_up;
    return p;
  }
  void top_up(int power) {
    p += power;
    record.pb(power);
...
int binary_search(int q) {
  ...
  while (r-l > 1) {
    int m = (l+r)/2;
    int power = player[rank_table[m]].update();
...
void incident() {
  ...
  if (type == 1) {
    ...
    int diff = player[pre].update() - player[a].update();
...
int main() {
  ...
  for (int i = 1; i <= N; i++) {
    ...
    for (int j : player[i].record) cout << ' ' << j;
    cout << '\n';
  }
  return 0;
}
